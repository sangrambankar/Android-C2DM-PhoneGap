...
document.addEventListener('deviceready', function() {
  ...
  //
  window.plugins.GCM.register("342624977232", "GCM_Event", GCM_Success, GCM_Fail );
...
GCM_Event(e)
{
  ...
  case 'registered':
    ...
    if ( gApp.gcmregid.length > 0 )
    {
    	...
    	device_id = e.regid;
      $("#app-status-ul").append('<li>REGISTERED -> REGID:' + device_id + "</li>");
...
function myFunction()
{
	var serverURL = 'http://192.168.2.35/capapp/mobilewebservice.asmx/getVarificationNativeCode';
	var veri_code = document.getElementById('verifycode').value;
	var load_data = {
		devicetype: platform,
		device_id: device_id
	};
	   $.ajax({
           ...
           url: serverURL,
           dataType: "json",
           data: JSON.stringify({"veri_code": veri_code,"platform":platform,"deviceid":device_id}),
